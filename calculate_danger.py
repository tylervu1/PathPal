...
# Aggregate danger values per block
block_danger_values = data.groupby(['street_name', 'LOCATION', 'LAT', 'LON'])['weighted_count'].sum().reset_index()

# Export results as a CSV file
block_danger_values.to_csv('block_danger_values.csv', columns=['LAT', 'LON', 'weighted_count'], index=False)
